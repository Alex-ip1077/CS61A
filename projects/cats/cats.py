...
def pick(paragraphs: list[str], select, k: int) -> str:
    ...
    """
    # BEGIN PROBLEM 1
    delegate = [xs for xs in paragraphs if select(xs)]
    if k >= len(delegate):
        return ''
    return delegate[k]
    # END PROBLEM 1
...
def about(keywords: list[str]):
    ...
    # BEGIN PROBLEM 2
    return lambda xs: any([x in split(lower(remove_punctuation(xs))) for x in keywords])
    # END PROBLEM 2
...
def accuracy(entered: str, source: str) -> float:
    ...
    if not source_words:
        return 0.0
    if len(entered_words) <= len(source_words):
        return 100.0 * len([entered_words[i] for i in range(len(entered_words)) if entered_words[i] == source_words[i]]) / len(entered_words)
    return 100.0 * len([entered_words[i] for i in range(len(source_words)) if entered_words[i] == source_words[i]]) / len(entered_words)
    # END PROBLEM 3
...
def autocorrect(entered_word: str, word_list: list[str], diff_function, limit: int) -> str:
    ...
    """
    # BEGIN PROBLEM 5
    x = min(word_list, key=lambda x: diff_function(entered_word, x, limit))
    return entered_word if entered_word in word_list or diff_function(entered_word, x, limit) > limit else x
    # END PROBLEM 5
...
def furry_fixes(entered: str, source: str, limit: int) -> int:
    ...
    if entered and source:
        ...
        return furry_fixes(entered[1:], source[1:], limit - 1) + 1 if limit else 1
    if not entered and source:
        return len(source) if len(source) <= limit else limit + 1
    if entered and not source:
        return len(entered) if len(entered) <= limit else limit + 1
    return 0
    # END PROBLEM 6
...
def report_progress(entered: list[str], source: list[str], user_id: int, upload) -> float:
    ...
    """
    # BEGIN PROBLEM 8
    cnt = max([k for k in range(len(entered) + 1) if all([entered[i] == source[i] for i in range(k)])])
...
def time_per_word(words: list[str], timestamps_per_player: list[list[int]]) -> dict:
    ...
    def times_helper(xs):
        ...
        if len(xs) == 2:
            return [xs[1] - xs[0]]
        return [xs[1] - xs[0]] + times_helper(xs[1:])
    times = [times_helper(xs) for xs in ts_by_player]
    # END PROBLEM 9
